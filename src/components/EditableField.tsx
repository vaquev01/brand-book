...
export function EditableField({ value, onSave, onDelete, multiline, className = "", readOnly, inline }: Props) {
  ...
  if (inline) {
    return (
      <span className="group/inline relative inline-flex items-center">
        <span className={className}>{value}</span>
        <span className="no-print absolute -top-3 -right-3 flex gap-1 opacity-0 group-hover/inline:opacity-100 transition-opacity z-10">
          <button
            type="button"
            onClick={() => { setDraft(value); setIsEditing(true); }}
            className="w-5 h-5 bg-white border shadow-sm flex items-center justify-center rounded-full text-gray-400 hover:text-gray-900 hover:bg-gray-50 transition"
            title="Editar"
          >
